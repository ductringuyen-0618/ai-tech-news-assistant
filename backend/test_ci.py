...
def test_news_and_learning_routers_registered():
    """The two article-feed endpoints (main feed + Learning feed) are
    both mounted -- a quick regression guard for `_safe_include_router`
    silently swallowing an import error."""
    from src.main import app
    ...
    assert "/api/learning/" in openapi_paths
    assert paths or openapi_paths  # sanity: app has *some* routes
